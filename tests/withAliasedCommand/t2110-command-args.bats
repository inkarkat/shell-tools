...
@test "alias definition via placeholders in --alias" {
    run -0 withAliasedCommand --name special --alias echo Go '{1}-{2}' for '{3}' and '{2}:' '{*}' \; -- special one two three
    assert_output 'Go one-two for three and two: one two three'
}

@test "quoting in alias definition via --alias" {
    run -0 withAliasedCommand --name special --alias echo Go '{1}-{2}' for '{3}' and '{2}:' '{*}' \; -- special 'the one' '/two\' "th'ee"
