...
@test "initial call with start three months prior passes all first 3 months" {
    setDate 2021-03-05
    run processPassedMonths --id ID --start-at 2020-12 --max-first 2 -- printf '[%s]-'
    [ $status -eq 0 ]
...
@test "subsequent call after 18 months passes first 2 months" {
    setDate 2020-03-05
    run processPassedMonths --id ID --start-at 2019-12 --max-first 2 -- printf '[%s]-'
    [ $status -eq 0 ]
