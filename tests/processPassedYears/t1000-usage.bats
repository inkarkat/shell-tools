...
@test "invalid option prints message and usage instructions" {
    run processPassedYears --invalid-option
    [ $status -eq 2 ]
...
@test "use of both initial-first and initial-last prints message and usage instructions" {
    run processPassedYears --id ID --initial-first 1 --initial-last 1
    [ $status -eq 2 ]
...
@test "use of both max-first and max-last prints message and usage instructions" {
    run processPassedYears --id ID --max-first 1 --max-last 1
    [ $status -eq 2 ]
